...
import com.speedb.utils.FormatVersion;
import com.speedb.utils.HashUtils;
import com.speedb.utils.LongPacker;
...
class StorageWriter {
    ...
    StorageWriter(Configuration configuration, OutputStream stream) {
        ...
        isCompressionEnabled = configuration.getBoolean(Configuration.COMPRESSION_ENABLED, false);

        // Create temp path folder
        tempFolder = TempUtils.createTempDir("speedbtempwriterdir");
    ...
    void put(byte[] key, byte[] inputValue)
            throws IOException {
        byte[] finalValue = inputValue;
        if (isCompressionEnabled) {
            // Compress using Snappy.
            finalValue = Snappy.compress(inputValue);
        }
        ...
        boolean sameValue = lastValue != null && Arrays.equals(finalValue, lastValue);
        ...
        if (!sameValue) {
            ...
            int valueSize = LongPacker.packInt(dataStream, finalValue.length);
            dataStream.write(finalValue);

            // Update data length
            dataLengths[keyLength] += valueSize + finalValue.length;

            // Update last value
            lastValues[keyLength] = finalValue;
            lastValuesLength[keyLength] = valueSize + finalValue.length;
    ...
    private void writeMetadata(DataOutputStream dataOutputStream)
        ...
        dataOutputStream.writeBoolean(config.getBoolean(Configuration.COMPRESSION_ENABLED));
